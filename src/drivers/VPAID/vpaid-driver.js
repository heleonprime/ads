...
window.getVPAIDAd = function () {
    ...
    VPAIDAd.prototype.initAd = function (width, height, viewMode, desiredBitrate, creativeData, environmentVars) {
        ...
        this.adUnitId = this.adParameters.blockId;
        l('adUnitId', this.adUnitId)
    ...
    VPAIDAd.prototype.addHeadScript = function () {
        ...
        let hs = document.querySelector("script[src=\"https:\/\/yandex.ru\/ads\/system\/context.js\"]");
        l('Already got script:', hs ? 1 : 0)
        if (!hs) {
    ...
    VPAIDAd.prototype.getAdUnitContainerId = function () {
        return o.adUnitDivIdPrefix + this.adUnitId;
    }

    VPAIDAd.prototype.addAdUnitContainer = function () {
        l('addAdUnitContainer started');
        let d = document.createElement('div');
        d.id = this.getAdUnitContainerId(this.adUnitId);
        this._slot.append(d);
    ...
    VPAIDAd.prototype.renderAdUnit = function () {
        ...
        window.yaContextCb.push(() => {
            l('Ya.Context.AdvManager', Ya.Context.AdvManager);
            Ya.Context.AdvManager.render({
                "blockId": _self.adUnitId,
                "renderTo": _self.getAdUnitContainerId(),
                onError: (data) => {
                    l('==> AdUnit ERROR fired');
                    l('Data: ', data); // type, code, text
                    let badErrors = [
                        'CONTAINER_NOT_FOUND',
                        'BLOCK_NOT_FOUND',
                        'PAGE_NOT_FOUND',
                    ...
                    if (data.type == 'error') {
                        if (badErrors.includes(data.code)) {
                            l('Really bad error fired, pulling AdError event');
                            _self.stopped = true;
                            clearTimeout(adUnitTimeout);
                            _self.eventHandler('AdError');
                        }
                    }
                },
                onRender: (data) => {
                    ...
                    l('Data: ', data);
                    _self.started = true;
                    _self.eventHandler('AdStarted');
                    _self.setAdUnitTimeout();
                    if (data.productType === 'direct') {
                        if (_self.impressionTimeoutDuration) {
                            _this.impressionTimeout = setTimeout(() => {
                                l('impressionTimeout');
                                _self.eventHandler('AdImpression');
                            }, _self.impressionTimeoutDuration);
                        }
                    }
                    else if (data.productType === 'video-creative-reach') {
                        _self.eventHandler('AdVideoStart');
                    }
                    else if (data.productType === '') {
                        l('Strange ad with empty type was loaded. Maybe there\'s a creative inside, maybe not. We can\'t handle it. Waiting for error?')
                    }
                },
                onClose: (data) => {
                    l('==> AdUnit "Close" fired');
                    l('Data: ', data);
                    clearTimeout(adUnitTimeout);
                    _self.stopped = true;
                    _self.eventHandler('AdStopped');
                },
                videoCallbacks: {
                    onAdStart: (adBreakType) => {
                        l('==> onAdStart', adBreakType);
                        _self.started = true;
                        _self.eventHandler('AdVideoStart');
                    },
                    onAdEnd: (adBreakType) => {
                        l('==> onAdEnd', adBreakType);
                        _self.stopped = true;
                        _self.eventHandler('AdStopped');
                        _self.eventHandler('AdVideoComplete');
                    },
                }
            })
        });
        l('window.yaContextCb after pushing the Ad:', window.yaContextCb);
    }
